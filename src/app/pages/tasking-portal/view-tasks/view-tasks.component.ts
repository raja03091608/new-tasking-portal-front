...
export class ViewTasksComponent implements OnInit {
  ...
  saveviewstatus() {
    ...
    console.log('commentform', this.extensionForm.value);
    let payload = {
      id: "",
      tasking: this.id,
      extension_no: this.extensionForm.get('extensionNumber').value,
      authority_letter: this.extensionForm.get('authorityLetterNumber').value,
      description: this.extensionForm.get('description').value,
      date: this.datepipe.transform(this.extensionForm.get('extendedTill').value, 'yyyy-MM-dd'),
  };
  
    this.api.postAPI( environment.API_URL + "transaction/extended/details/",payload).subscribe((res) => {
      if(res.status==environment.SUCCESS_CODE){
